# Adapted from vlax/models/big_vision/predict_fns.py
...
from vlax.models.big_vision import registry
...
def _decode_with_logp(
    ...
    *,
    ...
    bs_shardable = batch_size % jax.device_count() == 0
    ...
    for idx in range(max_decode_len):
        ...
        logits, cache = extend_cache(train_state["params"], cache, tokens, model=model)
...
def _decode_sample_output(state, logits, tok_mask, *, max_decode_len, sampler):
    ...
    # Sample tokens.
    sampled_tokens, sampled_logp = _sample_logits(
        logits, sampler=sampler, tok_mask=tok_mask
    )
...
def _prefill_cache(params, batch: VLABatch, *, model, max_decode_len):
    ...
    sequence_embeds, _ = model.apply(
        variables,
        sequence=sequence,
        data=data,
        data_mask=data_mask,
        method=model.make_inputs,
    )
    last_logits, variables = model.apply(
        variables,
        sequence_embeds.data,
        sequence_embeds.mask_exists,
        sequence_embeds.mask_ar,
        cache_size=sequence_embeds.data.shape[1] + max_decode_len,
...
def _sample_logits(logits, sampler, tok_mask=None):
    ...
    sampled_tokens = registry.Registry.lookup("vla_sampler." + sampler)(
...
@registry.Registry.register("vla_sampler.greedy")
def _greedy_sampling(*, logits, rng):
    del rng
    return jnp.argmax(logits, axis=-1)


@registry.Registry.register("vla_sampler.temperature")
def _temperature_sampling(t, *, logits, rng):
    return jax.random.categorical(rng, logits / t)


@registry.Registry.register("vla_sampler.nucleus")
