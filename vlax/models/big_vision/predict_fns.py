...
import vlax.models.big_vision.utils as u
...
def _extend_cache(params, cache, tokens, *, model):
    ...
    x, _, _ = model.apply(variables, tokens, method=model.embed_text)
    last_logits, variables = model.apply(
...
def _beam_decode(
    train_state, batch, *, model, devices, max_decode_len, eos_token, beam_size
):
